classdef ISpin_SAGA_exported < matlab.apps.AppBase

    % Properties that correspond to app components
    properties (Access = public)
        UIFigure                        matlab.ui.Figure
        RecordingSettingsPanel          matlab.ui.container.Panel
        ForcechDropDown                 matlab.ui.control.DropDown
        ForcechDropDownLabel            matlab.ui.control.Label
        DockcomDropDown                 matlab.ui.control.DropDown
        DockcomDropDownLabel            matlab.ui.control.Label
        RefreshrateDropDown             matlab.ui.control.DropDown
        RefreshrateDropDownLabel        matlab.ui.control.Label
        ChpergridDropDown               matlab.ui.control.DropDown
        ChpergridDropDownLabel          matlab.ui.control.Label
        ...
        InitializeSAGAButton            matlab.ui.control.Button
        DisconnectSAGAButton            matlab.ui.control.Button
    ...
    properties (Access = private)
        ...
        ComParameters       % Structure to save the parameters to interface with SAGA
    ...
    methods (Access = private)
        ...
        function Grid1CheckBoxValueChanged(app, event)
            if app.Grid1CheckBox.Value == 1 && str2double(app.ChpergridDropDown.Value) == 32
                app.EditField_1.Editable = 'on';
                app.Grid2CheckBox.Enable = 'on';
                app.RecordParameters.ngrid = 1;
                app.InitializeSAGAButton.Enable = 'on';
                app.ComParameters.nchannel = 41;
                app.ComParameters.gridchannels = 2:1:33;
            elseif app.Grid1CheckBox.Value == 1 && str2double(app.ChpergridDropDown.Value) == 64
                app.EditField_1.Editable = 'on';
                app.RecordParameters.ngrid = 1;
                app.InitializeSAGAButton.Enable = 'on';
                app.ComParameters.nchannel = 73;
                app.ComParameters.gridchannels = 2:1:65; 
            else
                ...
                app.InitializeSAGAButton.Enable = 'off';
            end
        end
        ...
        function Grid2CheckBoxValueChanged(app, event)
            if app.Grid2CheckBox.Value == 1
                ...
                app.RecordParameters.ngrid = 1;
                app.ComParameters.nchannel = 41;
                app.ComParameters.gridchannels = 2:33;
            end
        end
        ...
        % Value changed function: ChpergridDropDown
        function ChpergridDropDownValueChanged(app, event)
            if str2double(app.ChpergridDropDown.Value) == 32
                if app.Grid1CheckBox.Value == 1
                    app.Grid2CheckBox.Enable = 'on';
                end
            else
                app.Grid2CheckBox.Value = 0;
                app.Grid2CheckBox.Enable = 'off';
                app.EditField_2.Value = '';
                app.EditField_2.Editable = 'off';
                if app.Grid1CheckBox.Value == 1
        ...
        % Button pushed function: InitializeSAGAButton
        function InitializeSAGAButtonPushed(app, event)
            app.ComParameters.fsamp = str2double(app.FrequencyDropDown.Value);
            if app.ComParameters.fsamp == 4096
                Dividers = 0;
            elseif app.ComParameters.fsamp == 2048
                Dividers = 1;
            elseif app.ComParameters.fsamp == 1024
                Dividers = 2;
            else
                Dividers = 3;
            end
            ...
            % the example.
            if exist('app.ComParameters.lib', 'var')
                disp('[N/A] Ensure library is cleaned...')
                app.ComParameters.lib.cleanUp();
            end

            % Initialize the library
            app.ComParameters.lib = TMSiSAGA.Library();

            % Specify desired device configuration
            app.ComParameters.deviceConfig = struct('ImpedanceMode', false,...
                'ReferenceMethod', 'common', ...
                'BaseSampleRate', 4096, ...
                'Dividers', {{'uni', Dividers; 'bip', Dividers; 'aux', Dividers;}});

            % Specify desired channel configuration
            app.ComParameters.channelConfig=struct('uni', [1:str2double(app.ChpergridDropDown.Value)*app.RecordParameters.ngrid], 'bip', [1:4], 'aux', [1:3], 'acc',0, 'dig', 0);

            % Get a single device from the connected devices
            % possible interfaces 'usb'/'network' and 'electrical'/'optical/'wifi'
            try
                % Get the device
                app.ComParameters.device = app.ComParameters.lib.getFirstAvailableDevice('usb', app.DockcomDropDown.Value);
            catch
                % Get the device
                app.ComParameters.device = app.ComParameters.lib.getFirstAvailableDevice('usb', 'electrical');

                % Open a connection to the device
                app.ComParameters.device.connect();
    
                % Change interface with docking station
                app.ComParameters.device.changeDataRecorderInterfaceTo(app.DockcomDropDown.Value)

                % Get the device with selected docking interface
                app.ComParameters.device = app.ComParameters.lib.getFirstAvailableDevice('usb', app.DockcomDropDown.Value);
            end
            ...
            app.ComParameters.device.setDeviceConfig(app.ComParameters.deviceConfig);

            % Set channel configuration
            app.ComParameters.device.setChannelConfig(app.ComParameters.channelConfig);

            % Update the UI for the next acquisition
            app.GriddisplayedDropDown.Enable = 'on';
            app.InitializeSAGAButton.Enable = 'off';
            app.InitializeSAGAButton_2.Enable = 'off';
            app.DisconnectSAGAButton.Enable = 'on';
            app.DisconnectSAGAButton_2.Enable = 'on';
        ...
        % Button pushed function: DisconnectSAGAButton
        function DisconnectSAGAButtonPushed(app, event)
            % Stop sampling on the device
            app.ComParameters.device.stop();

            % Disconnect from device
            app.ComParameters.device.disconnect();

            % clean up and unload the library
            app.ComParameters.lib.cleanUp();

            app.InitializeSAGAButton.Enable = 'on';
            app.InitializeSAGAButton_2.Enable = 'on';
            app.DisconnectSAGAButton.Enable = 'off';
            app.DisconnectSAGAButton_2.Enable = 'off';
            app.StartVisualizationCheckEMGButton.Enable = 'off';
            app.StartVisualizationChecknoiseButton.Enable = 'off';
        ...
        function StartVisualizationChecknoiseButtonPushed(app, event)
            ...
            nwin = floor(15*str2double(app.FrequencyDropDown.Value));
            nwinbuffmax = floor(str2double(app.FrequencyDropDown.Value)*4.8);
            i = 1;
            nwinbuff = 1;

            % Set the properties of the filter
            [c,d] = butter(3,10/(str2double(app.FrequencyDropDown.Value)/2),'high');
            ...
            try
                ...
                EMG = EMG(:,1:i);
                % Check EMG noise
                Noisecheck = zeros(str2double(app.ChpergridDropDown.Value),1);
                for j = 1:str2double(app.ChpergridDropDown.Value)
                    Noisecheck(j) = rms(EMGfilter(str2double(app.FrequencyDropDown.Value):end-str2double(app.FrequencyDropDown.Value),(str2double(app.GriddisplayedDropDown.Value)-1)*str2double(app.ChpergridDropDown.Value)+j)'/1000);
                end
                bar(app.UIAxes3,Noisecheck)
                app.UIAxes3.XLim = [0 str2double(app.ChpergridDropDown.Value)+1];
        ...
        function StartVisualizationCheckEMGButtonPushed(app, event)
            ...
            nwin = floor(30*str2double(app.FrequencyDropDown.Value));
            nwinbuffmax = floor(str2double(app.FrequencyDropDown.Value)*4.8);
            i = 1;
            nwinbuff = 1;
            ...
            try
                ...
                for i = 1:app.RecordParameters.ngrid
                    EMGcheck = zeros(8,size(EMGfilter,2)*str2double(app.ChpergridDropDown.Value)/8);
                    for j = 1:str2double(app.ChpergridDropDown.Value)/8
                        EMGcheck(:,(j-1)*size(EMGfilter,1)+1:j*size(EMGfilter,1)) = EMGfilter(:,(i-1)*str2double(app.ChpergridDropDown.Value)+j*8-7:(i-1)*str2double(app.ChpergridDropDown.Value)+j*8)';
                    end
                    offset2 = ones(8,size(EMGcheck,2)) .* (1:1:8)';
                    EMGcheck = EMGcheck * 0.001 + offset2;
                    plot(app.UIAxes3,EMGcheck')
                    line(app.UIAxes3,[size(EMG,2) size(EMG,2)], [0 9], 'LineWidth', 2, 'Color', [0.81 0.50 1.00])
                    line(app.UIAxes3,[2*size(EMG,2) 2*size(EMG,2)], [0 9], 'LineWidth', 2, 'Color', [0.81 0.50 1.00])
                    line(app.UIAxes3,[3*size(EMG,2) 3*size(EMG,2)], [0 9], 'LineWidth', 2, 'Color', [0.81 0.50 1.00])
                    if str2double(app.ChpergridDropDown.Value) == 64
                        line(app.UIAxes3,[4*size(EMG,2) 4*size(EMG,2)], [0 9], 'LineWidth', 2, 'Color', [0.81 0.50 1.00])
                        line(app.UIAxes3,[5*size(EMG,2) 5*size(EMG,2)], [0 9], 'LineWidth', 2, 'Color', [0.81 0.50 1.00])
                        line(app.UIAxes3,[6*size(EMG,2) 6*size(EMG,2)], [0 9], 'LineWidth', 2, 'Color', [0.81 0.50 1.00])
                        line(app.UIAxes3,[7*size(EMG,2) 7*size(EMG,2)], [0 9], 'LineWidth', 2, 'Color', [0.81 0.50 1.00]) 
                    end
                    app.UIAxes3.XLim = [0 size(EMGcheck,2)];
                    app.UIAxes3.YLim = [0 9];
                    xlabel(app.UIAxes3,'Columns');
                    ylabel(app.UIAxes3,'Rows');
                    drawnow
                    
                    if str2double(app.ChpergridDropDown.Value) == 32
                        prompt   = {'col #1', 'col #2', 'col #3', 'col #4'};
                    else
                        prompt   = {'col #1', 'col #2', 'col #3', 'col #4', 'col #5', 'col #6', 'col #7', 'col #8'};
                    end
        ...
        function OffsetCheckBoxValueChanged(app, event)
            if app.OffsetCheckBox.Value == 1
                ...
                nwin = (3*str2double(app.FrequencyDropDown.Value));
                i = 1;

                % Pre allocate an empty vector
                force = zeros(1,nwin+1000);
                ...
                try
                    ...
                    while i <= nwin
                        [samples, num_sets] = app.ComParameters.device.sample();
                        if num_sets > 0
                            force(:,i:i+num_sets-1) = samples(app.ComParameters.gridchannels(end)+4+str2double(app.ForcechDropDown.Value), :);
        ...
        function StartMVCButtonPushed(app, event)
            ...
            MVClight = [MVCBeg;MVCtrial;MVCrest;MVCtrial;MVCrest;MVCtrial;zeros(1000,1)];     
            ...
            Time = [TimeBeg Timetrial Timerest Timetrial Timerest Timetrial zeros(1,1000)];

            % Pre allocate empty matrices
            EMG = zeros(app.RecordParameters.ngrid*str2double(app.ChpergridDropDown.Value),trialength+1000);
            Force = zeros(1,trialength+1000);
            ...
            try
                ...
                while i <= nwin
                    ...
                    if num_sets > 0
                        EMG(:,i:i+num_sets-1) = samples(app.ComParameters.gridchannels, :);
                        Force(:,i:i+num_sets-1) = samples(app.ComParameters.gridchannels(end)+4+str2double(app.ForcechDropDown.Value), :) - app.RecordParameters.offsetforce;

                        if i+num_sets-1 > str2double(app.FrequencyDropDown.Value)/8
                            app.Gauge.Value = abs(mean(Force(i+num_sets-1-str2double(app.FrequencyDropDown.Value)/8:i+num_sets-1))) * 0.0005;
                        else
                            app.Gauge.Value = abs(mean(Force(1:i+num_sets-1))) * 0.0005;
                        end
                        app.EditField_MVC.Value = num2str(round(Time(i+num_sets-1),1));
                        if MVClight(i+num_sets-1) == 1
                ...
                % if no force sensor
                if app.RecordParameters.MVC == 0
                    app.RecordParameters.MVC = 1;
                end
        ...
        function StartButtonPushed(app, event)
            ...
            nwin = app.DecompParameters.durationtraining-floor(str2double(app.FrequencyDropDown.Value)/32);
            nwinbuffmax = floor(str2double(app.FrequencyDropDown.Value)*5);
            i = 1;
            [c,d] = butter(3,10/(str2double(app.FrequencyDropDown.Value)/2),'high');
            ...
            EMG = zeros(app.RecordParameters.ngrid*str2double(app.ChpergridDropDown.Value),app.DecompParameters.durationtraining);
            Force = NaN(1,app.DecompParameters.durationtraining);
            ForceDisp = NaN(1,app.DecompParameters.durationtraining);
            ...
            try
                ...
                while i <= nwin - nwinbuffmax
                    ...
                    if num_sets > 0
                        ...
                        if i <= nwinbuffmax
                            ForceDisp(:,1:i+num_sets-1) = movmean(Force(:,1:i+num_sets-1),(str2double(app.FrequencyDropDown.Value)/str2double(app.RefreshrateDropDown.Value)));
                            plot(app.UIAxes3, timetot(1:str2double(app.FrequencyDropDown.Value)*10), abs(ForceDisp(1:str2double(app.FrequencyDropDown.Value)*10))/app.RecordParameters.MVC *100, ...
                                ...
                                timetot(i+num_sets-1), abs(ForceDisp(i+num_sets-1))/app.RecordParameters.MVC * 100, 'o', 'MarkerSize', 20, 'LineWidth', 3, 'Color', [0.38 0.78 0.75]);
                            ...
                            plot(app.UIAxes, timetot, app.DecompParameters.target, timetot(i+num_sets-1), app.DecompParameters.target(i+num_sets-1), 'o', 'LineWidth', 2, 'MarkerSize', 20, 'Color', [0.56 0.62 0.85]);
                            drawnow limitrate
                        else
                            if i+num_sets-1+nwinbuffmax > app.DecompParameters.durationtraining
                                ForceDisp(:,i+num_sets-nwinbuffmax:i+num_sets-1) = movmean(Force(:,i+num_sets-nwinbuffmax:i+num_sets-1),round(str2double(app.FrequencyDropDown.Value)/str2double(app.ChpergridDropDown.Value)));
                                plot(app.UIAxes3, timetot(i+num_sets-nwinbuffmax:end), abs(ForceDisp(i+num_sets-nwinbuffmax:app.DecompParameters.durationtraining))/app.RecordParameters.MVC *100, ...
                                    timetot(i+num_sets-nwinbuffmax:end), app.DecompParameters.target(i+num_sets-nwinbuffmax:app.DecompParameters.durationtraining), ...
                                    timetot(i+num_sets-1), abs(Force(i+num_sets-1))/app.RecordParameters.MVC *100, 'o', 'MarkerSize', 20, 'LineWidth', 3, 'Color', [0.38 0.78 0.75]);
                                app.UIAxes3.XLim = [timetot(i+num_sets-nwinbuffmax) timetot(end)];
                                app.UIAxes3.YLim = [0 app.MVCTargetEditField.Value + 3];
                                plot(app.UIAxes, timetot, app.DecompParameters.target, timetot(i+num_sets-1), app.DecompParameters.target(i+num_sets-1), 'o', 'LineWidth', 2, 'MarkerSize', 20, 'Color', [0.56 0.62 0.85]);
                                drawnow limitrate
                            else
                                ForceDisp(:,i+num_sets-nwinbuffmax:i+num_sets-1) = movmean(Force(:,i+num_sets-nwinbuffmax:i+num_sets-1),round(str2double(app.FrequencyDropDown.Value)/str2double(app.ChpergridDropDown.Value)));
                                plot(app.UIAxes3, timetot(i+num_sets-nwinbuffmax:i+num_sets-1+nwinbuffmax), abs(ForceDisp(i+num_sets-nwinbuffmax:i+num_sets-1+nwinbuffmax))/app.RecordParameters.MVC *100, ...
                                    timetot(i+num_sets-nwinbuffmax:i+num_sets-1+nwinbuffmax), app.DecompParameters.target(i+num_sets-nwinbuffmax:i+num_sets-1+nwinbuffmax), ...
                                    timetot(i+num_sets-1), abs(Force(i+num_sets-1))/app.RecordParameters.MVC *100, 'o', 'MarkerSize', 20, 'LineWidth', 3, 'Color', [0.38 0.78 0.75]);
                                app.UIAxes3.XLim = [timetot(i+num_sets-nwinbuffmax) timetot(i+num_sets-1+nwinbuffmax)];
                                app.UIAxes3.YLim = [0 app.MVCTargetEditField.Value + 3];
                                plot(app.UIAxes, timetot, app.DecompParameters.target, timetot(i+num_sets-1), app.DecompParameters.target(i+num_sets-1), 'o', 'LineWidth', 2, 'MarkerSize', 20, 'Color', [0.56 0.62 0.85]);
                                drawnow limitrate
                            end
                ...
                EMG = EMG(:,1:i);
                Force = Force(:,1:i);
                signal.target = app.DecompParameters.target(:,1:i);


                offsetMU = 0;

                for i = 1:app.RecordParameters.ngrid
                    SIG = filtfilt(c,d,EMG((i-1)*str2double(app.ChpergridDropDown.Value)+1:i*str2double(app.ChpergridDropDown.Value),25*str2double(app.FrequencyDropDown.Value):25*str2double(app.FrequencyDropDown.Value)+app.DurationtrainingEditField.Value*str2double(app.FrequencyDropDown.Value))');
                    ...
                    for j = 1:app.NbIterationsEditField.Value
                        ...
                        if length(spikes) > 10 % If the number of peaks is low, skip the second algorithm
                            ...
                            % if SIL(j) > app.SILthresholdEditField.Value
                    ...
                    if size(MUFilters,2) > 0

                        % Get the pulse train for the entire signal
                        EMGfilter = filtfilt(c,d,EMG((i-1)*str2double(app.ChpergridDropDown.Value)+1:i*str2double(app.ChpergridDropDown.Value),:)');
                        [PulseT, Distime, ~] = getPulseT(EMGfilter', app.RecordParameters.DiscardChannels{i}, MUFilters, str2double(app.FrequencyDropDown.Value));

                        % Remove duplicates
                        [~, Distime] = remduplicates(PulseT, Distime, Distime, exFactor, 0.00025, 0.3, str2double(app.FrequencyDropDown.Value));

                        % Get the decomposition parameters for the biofeedback
                        app.DecompParameters.MUFilters{i} = getMUfilters(EMGfilter', app.RecordParameters.DiscardChannels{i}, Distime);
                        [PulseT, Distime, ~] = getPulseT(EMGfilter', app.RecordParameters.DiscardChannels{i}, app.DecompParameters.MUFilters{i}, str2double(app.FrequencyDropDown.Value));
                        [app.DecompParameters.extensionfactor(i), app.DecompParameters.iReSIG{i}, app.DecompParameters.norm{i}, app.DecompParameters.centroid{i}] = getonlineparameters(EMGfilter', app.RecordParameters.DiscardChannels{i}, app.DecompParameters.MUFilters{i}, str2double(app.FrequencyDropDown.Value));
                        ...
                        if str2double(app.ChpergridDropDown.Value) == 32
                            signal.gridname{i} = 'GR08.75MM0408';
                        else
                            signal.gridname{i} = 'GR08.75MM0808';
                        end
                        signal.SIL{i} = SIL;
                        signal.COV{i} = COV;
                ...
                signal.data = EMG;
                Force = movmean(Force,(str2double(app.FrequencyDropDown.Value)/str2double(app.RefreshrateDropDown.Value)));
        ...
        function ImportMUfiltersButtonPushed(app, event)
            ...
            for i = 1:file.signal.ngrid
                ...
                for j = 1:size(file.edition.Pulsetrain{i},1)
                ...
                % decomposition
                [c,d] = butter(3,10/(str2double(app.FrequencyDropDown.Value)/2),'high');
                dataFilter = filtfilt(c,d,file.signal.data((i-1)*str2double(app.ChpergridDropDown.Value)+1:i*str2double(app.ChpergridDropDown.Value),:)');
                app.DecompParameters.MUFilters{i} = getMUfilters(dataFilter', app.DecompParameters.EMGmask{i}, Distime);
                [app.DecompParameters.extensionfactor(i), app.DecompParameters.iReSIG{i}, app.DecompParameters.norm{i}, app.DecompParameters.centroid{i}] = getonlineparameters(dataFilter', app.DecompParameters.EMGmask{i}, app.DecompParameters.MUFilters{i}, str2double(app.FrequencyDropDown.Value));
                clearvars Distime
            end
        ...
        function StartRasterPlotwithMUsButtonPushed(app, event)
            ...
            nwin = app.OnlineParameters.durationonline;
            nwinbuffmax = 5*str2double(app.RefreshrateDropDown.Value);
            k = 1;
            i = 1;
            [c,d] = butter(3,10/(str2double(app.FrequencyDropDown.Value)/2),'high');
            
            % Pre allocate empty matrices
            Time = linspace(0,nwin/str2double(app.FrequencyDropDown.Value),nwin);
            EMG = zeros(str2double(app.ChpergridDropDown.Value),nwin);
            EMGsave = zeros(app.RecordParameters.ngrid*str2double(app.ChpergridDropDown.Value),nwin);
            Force = NaN(1,nwin);
            EMGtmp{1} = zeros(str2double(app.ChpergridDropDown.Value),(str2double(app.FrequencyDropDown.Value)/str2double(app.RefreshrateDropDown.Value)));
            esample2{1} = zeros((str2double(app.ChpergridDropDown.Value)-sum(app.DecompParameters.EMGmask{idxmuscle1}))*app.DecompParameters.extensionfactor(idxmuscle1),app.DecompParameters.extensionfactor(idxmuscle1)-1);
            Distime{1} = zeros(nwin, size(app.DecompParameters.MUFilters{idxmuscle1},2));
            signal.Pulsetrain{1} = zeros(size(app.DecompParameters.MUFilters{idxmuscle1},2), nwin);
            noise_centroids{1} = ones((str2double(app.FrequencyDropDown.Value)/str2double(app.RefreshrateDropDown.Value)),size(app.DecompParameters.MUFilters{idxmuscle1},2)) .* (app.DecompParameters.centroid{idxmuscle1}(:,1)');
            spike_centroids{1} = ones((str2double(app.FrequencyDropDown.Value)/str2double(app.RefreshrateDropDown.Value)),size(app.DecompParameters.MUFilters{idxmuscle1},2)) .* (app.DecompParameters.centroid{idxmuscle1}(:,2)');
            ...
            forcefb = zeros(1,nwin/(str2double(app.FrequencyDropDown.Value)/str2double(app.RefreshrateDropDown.Value)));
            feedback = zeros(1,nwin/(str2double(app.FrequencyDropDown.Value)/str2double(app.RefreshrateDropDown.Value)));
            ...
            try
                ...
                app.ComParameters.device.start();
                while i <= nwin - str2double(app.FrequencyDropDown.Value)*5
                    [samples, num_sets] = app.ComParameters.device.sample();
                    if num_sets > 0
                        EMG(:,i:i+num_sets-1) = samples(app.ComParameters.gridchannels((idxmuscle1-1)*str2double(app.ChpergridDropDown.Value)+1:idxmuscle1*str2double(app.ChpergridDropDown.Value)), :);
                        EMGsave(:,i:i+num_sets-1) = samples(app.ComParameters.gridchannels, :);
                        Force(:,i:i+num_sets-1) = samples(app.ComParameters.gridchannels(end)+4+str2double(app.ForcechDropDown.Value),:) - app.RecordParameters.offsetforce;


                        if k*(str2double(app.FrequencyDropDown.Value)/str2double(app.RefreshrateDropDown.Value)) <= i+num_sets-1
                            forcefb(k) = mean(abs(Force((k-1)*(str2double(app.FrequencyDropDown.Value)/str2double(app.RefreshrateDropDown.Value))+1:k*(str2double(app.FrequencyDropDown.Value)/str2double(app.RefreshrateDropDown.Value)))));
                            feedback(k) = mean(app.OnlineParameters.targetonline((k-1)*(str2double(app.FrequencyDropDown.Value)/str2double(app.RefreshrateDropDown.Value))+1:k*(str2double(app.FrequencyDropDown.Value)/str2double(app.RefreshrateDropDown.Value))));

                            EMGtmp{1} = filtfilt(c,d,EMG(app.DecompParameters.EMGmask{idxmuscle1}==0, (k-1)*(str2double(app.FrequencyDropDown.Value)/str2double(app.RefreshrateDropDown.Value))+1:k*(str2double(app.FrequencyDropDown.Value)/str2double(app.RefreshrateDropDown.Value)))');

                            [signal.Pulsetrain{1}(:,(k-1)*(str2double(app.FrequencyDropDown.Value)/str2double(app.RefreshrateDropDown.Value))+1:k*(str2double(app.FrequencyDropDown.Value)/str2double(app.RefreshrateDropDown.Value))), Distime{1}((k-1)*(str2double(app.FrequencyDropDown.Value)/str2double(app.RefreshrateDropDown.Value))+1:k*(str2double(app.FrequencyDropDown.Value)/str2double(app.RefreshrateDropDown.Value)), :), esample2{1}] = getspikesonline(EMGtmp{1}', app.DecompParameters.extensionfactor(idxmuscle1), esample2{1}, MUfilt{1}, app.DecompParameters.norm{idxmuscle1}, noise_centroids{1}, spike_centroids{1}, (str2double(app.FrequencyDropDown.Value)/str2double(app.RefreshrateDropDown.Value)), str2double(app.FrequencyDropDown.Value));
                            ...
                            if k < nwinbuffmax
                                ...
                                plot(app.UIAxes4, Time((k-1)*str2double(app.FrequencyDropDown.Value)/str2double(app.RefreshrateDropDown.Value)+1-str2double(app.FrequencyDropDown.Value)*2.5:str2double(app.FrequencyDropDown.Value)*2.5+(k-1)*str2double(app.FrequencyDropDown.Value)/str2double(app.RefreshrateDropDown.Value)), Distime{1}((k-1)*str2double(app.FrequencyDropDown.Value)/str2double(app.RefreshrateDropDown.Value)+1-str2double(app.FrequencyDropDown.Value)*2.5:str2double(app.FrequencyDropDown.Value)*2.5+(k-1)*str2double(app.FrequencyDropDown.Value)/str2double(app.RefreshrateDropDown.Value),:).*MUoffset, '|', 'LineWidth', 4, 'MarkerSize', 20, 'Color', [0.56 0.62 0.85])
                                app.UIAxes4.XLim = [Time((k-1)*str2double(app.FrequencyDropDown.Value)/str2double(app.RefreshrateDropDown.Value)+1-str2double(app.FrequencyDropDown.Value)*2.5) Time(str2double(app.FrequencyDropDown.Value)*2.5+(k-1)*str2double(app.FrequencyDropDown.Value)/str2double(app.RefreshrateDropDown.Value))];
                                plot(app.UIAxes5, 1, forcefb(k)/app.RecordParameters.MVC * 100,'o', 'LineWidth', 4, 'MarkerSize', 20, 'Color', [0.9412 0.9412 0.9412])
                                line(app.UIAxes5,[0 2], [feedback(k)*0.95 feedback(k)*0.95],'LineWidth', 4, 'Color', [0.9412 0.9412 0.9412])
                                line(app.UIAxes5,[0 2], [feedback(k)*1.05 feedback(k)*1.05],'LineWidth', 4, 'Color', [0.9412 0.9412 0.9412])
                            end
                ...
                Force = movmean(Force,str2double(app.RefreshrateDropDown.Value));
                signal.path = Force;
                signal.fsamp = str2double(app.FrequencyDropDown.Value);
        ...
        function StartQuadrantwithCSTsButtonPushed(app, event)
            ...
            EMGsave = zeros(app.RecordParameters.ngrid*str2double(app.ChpergridDropDown.Value),nwin);
            Force = NaN(1,nwin);
            Time = linspace(0,nwin/str2double(app.FrequencyDropDown.Value), floor(nwin*str2double(app.RefreshrateDropDown.Value)/str2double(app.FrequencyDropDown.Value)));
            EMGtmp{1} = zeros(str2double(app.ChpergridDropDown.Value),(str2double(app.FrequencyDropDown.Value)/str2double(app.RefreshrateDropDown.Value)));
            esample2{1} = zeros((str2double(app.ChpergridDropDown.Value)-sum(app.DecompParameters.EMGmask{idxmuscle1}))*app.DecompParameters.extensionfactor(idxmuscle1),app.DecompParameters.extensionfactor(idxmuscle1)-1);
            Distime{1} = zeros(nwin, size(app.DecompParameters.MUFilters{idxmuscle1},2));
            signal.Pulsetrain{1} = zeros(size(app.DecompParameters.MUFilters{idxmuscle1},2), nwin);
            ...
            spike_centroids{1} = ones((str2double(app.FrequencyDropDown.Value)/str2double(app.RefreshrateDropDown.Value)),size(app.DecompParameters.MUFilters{idxmuscle1},2)) .* (app.DecompParameters.centroid{idxmuscle1}(:,2)');
            MUfilt{1} = app.DecompParameters.MUFilters{idxmuscle1}' * app.DecompParameters.iReSIG{idxmuscle1};
            DR = NaN(nwin*str2double(app.RefreshrateDropDown.Value)/str2double(app.FrequencyDropDown.Value), size(app.DecompParameters.MUFilters{idxmuscle1},2));
            forcefb = zeros(1,nwin/(str2double(app.FrequencyDropDown.Value)/str2double(app.RefreshrateDropDown.Value)));
            feedback = zeros(1,nwin/(str2double(app.FrequencyDropDown.Value)/str2double(app.RefreshrateDropDown.Value)));
                ...
                while i <= nwin - str2double(app.FrequencyDropDown.Value)*5
                    ...
                    if num_sets > 0
                        ...
                        if k*(str2double(app.FrequencyDropDown.Value)/str2double(app.RefreshrateDropDown.Value)) <= i+num_sets-1
                            ...
                            if k < nwinbuffmax
                                DR(k,1) = sum(Distime{1}(1:k*(str2double(app.FrequencyDropDown.Value)/str2double(app.RefreshrateDropDown.Value)), 1:idxhMU) > 0, 'all');
                                DR(k,2) = sum(Distime{1}(1:k*(str2double(app.FrequencyDropDown.Value)/str2double(app.RefreshrateDropDown.Value)), idxhMU+1:end) > 0, 'all');
                                plot(app.UIAxes4, DR(1:10*str2double(app.RefreshrateDropDown.Value),1), DR(1:10*str2double(app.RefreshrateDropDown.Value),2), DR(k,1) , DR(k,2),'o', 'LineWidth', 4, 'MarkerSize', 20, 'Color', [0.56 0.62 0.85])
                                plot(app.UIAxes5, 1, forcefb(k)/app.RecordParameters.MVC * 100,'o', 'LineWidth', 4, 'MarkerSize', 20, 'Color', [0.9412 0.9412 0.9412])
                                line(app.UIAxes5,[0 2], [feedback(k)*0.95 feedback(k)*0.95],'LineWidth', 4, 'Color', [0.9412 0.9412 0.9412])
                                line(app.UIAxes5,[0 2], [feedback(k)*1.05 feedback(k)*1.05],'LineWidth', 4, 'Color', [0.9412 0.9412 0.9412])
                            else
                                DR(k,1) = sum(Distime{1}((k-str2double(app.RefreshrateDropDown.Value)+1)*(str2double(app.FrequencyDropDown.Value)/str2double(app.RefreshrateDropDown.Value))+1:k*(str2double(app.FrequencyDropDown.Value)/str2double(app.RefreshrateDropDown.Value)), 1:idxhMU) > 0, 'all');
                                DR(k,2) = sum(Distime{1}((k-str2double(app.RefreshrateDropDown.Value)+1)*(str2double(app.FrequencyDropDown.Value)/str2double(app.RefreshrateDropDown.Value))+1:k*(str2double(app.FrequencyDropDown.Value)/str2double(app.RefreshrateDropDown.Value)), idxhMU+1:end) > 0, 'all');
                                DR(k,:) = mean(DR(k-(app.SmoothingEditField.Value-1):k,:), 1);
                                plot(app.UIAxes4, DR(k-5*str2double(app.RefreshrateDropDown.Value)+1:k+5*str2double(app.RefreshrateDropDown.Value),1), DR(k-5*str2double(app.RefreshrateDropDown.Value)+1:k+5*str2double(app.RefreshrateDropDown.Value),2), DR(k,1) , DR(k,2),'o', 'LineWidth', 4, 'MarkerSize', 20, 'Color', [0.56 0.62 0.85])
                                plot(app.UIAxes5, 1, forcefb(k)/app.RecordParameters.MVC * 100,'o', 'LineWidth', 4, 'MarkerSize', 20, 'Color', [0.9412 0.9412 0.9412])
                                line(app.UIAxes5,[0 2], [feedback(k)*0.95 feedback(k)*0.95],'LineWidth', 4, 'Color', [0.9412 0.9412 0.9412])
                                line(app.UIAxes5,[0 2], [feedback(k)*1.05 feedback(k)*1.05],'LineWidth', 4, 'Color', [0.9412 0.9412 0.9412])
                            end
        ...
        function StartQuadrantwithMUsButtonPushed(app, event)
            ...
            EMG{1} = zeros(str2double(app.ChpergridDropDown.Value),nwin);
            EMGtmp{1} = zeros(str2double(app.ChpergridDropDown.Value),(str2double(app.FrequencyDropDown.Value)/str2double(app.RefreshrateDropDown.Value)));
            esample2{1} = zeros((str2double(app.ChpergridDropDown.Value)-sum(app.DecompParameters.EMGmask{idxmuscle1}))*app.DecompParameters.extensionfactor(idxmuscle1),app.DecompParameters.extensionfactor(idxmuscle1)-1);
            Distime{1} = zeros(nwin, size(app.DecompParameters.MUFilters{idxmuscle1},2));
            signal.Pulsetrain{1} = zeros(size(app.DecompParameters.MUFilters{idxmuscle1},2), nwin);
            ...
            EMG{2} = zeros(str2double(app.ChpergridDropDown.Value),nwin);
            EMGtmp{2} = zeros(str2double(app.ChpergridDropDown.Value),(str2double(app.FrequencyDropDown.Value)/str2double(app.RefreshrateDropDown.Value)));
            esample2{2} = zeros((str2double(app.ChpergridDropDown.Value)-sum(app.DecompParameters.EMGmask{idxmuscle2}))*app.DecompParameters.extensionfactor(idxmuscle2),app.DecompParameters.extensionfactor(idxmuscle2)-1);
            Distime{2} = zeros(nwin, size(app.DecompParameters.MUFilters{idxmuscle2},2));
            signal.Pulsetrain{2} = zeros(size(app.DecompParameters.MUFilters{idxmuscle2},2), nwin);
            noise_centroids{2} = ones((str2double(app.FrequencyDropDown.Value)/str2double(app.RefreshrateDropDown.Value)),size(app.DecompParameters.MUFilters{idxmuscle2},2)) .* (app.DecompParameters.centroid{idxmuscle2}(:,1)');
            spike_centroids{2} = ones((str2double(app.FrequencyDropDown.Value)/str2double(app.RefreshrateDropDown.Value)),size(app.DecompParameters.MUFilters{idxmuscle2},2)) .* (app.DecompParameters.centroid{idxmuscle2}(:,2)');
            MUfilt{2} = app.DecompParameters.MUFilters{idxmuscle2}' * app.DecompParameters.iReSIG{idxmuscle2};

            DR{1} = NaN(nwin*str2double(app.RefreshrateDropDown.Value)/str2double(app.FrequencyDropDown.Value), size(app.DecompParameters.MUFilters{idxmuscle1},2));
            DR{2} = NaN(nwin*str2double(app.RefreshrateDropDown.Value)/str2double(app.FrequencyDropDown.Value), size(app.DecompParameters.MUFilters{idxmuscle2},2));
            forcefb = zeros(1,nwin/(str2double(app.FrequencyDropDown.Value)/str2double(app.RefreshrateDropDown.Value)));
            feedback = zeros(1,nwin/(str2double(app.FrequencyDropDown.Value)/str2double(app.RefreshrateDropDown.Value)));
            ...
            try
                ...
                while i <= nwin - str2double(app.FrequencyDropDown.Value)*5
                    [samples, num_sets] = app.ComParameters.device.sample();
                    if num_sets > 0
                        EMG{1}(:,i:i+num_sets-1) = samples(app.ComParameters.gridchannels((idxmuscle1-1)*str2double(app.ChpergridDropDown.Value)+1:idxmuscle1*str2double(app.ChpergridDropDown.Value)), :);
                        EMG{2}(:,i:i+num_sets-1) = samples(app.ComParameters.gridchannels((idxmuscle2-1)*str2double(app.ChpergridDropDown.Value)+1:idxmuscle2*str2double(app.ChpergridDropDown.Value)), :);
                        EMGsave(:,i:i+num_sets-1) = samples(app.ComParameters.gridchannels, :);
                        Force(:,i:i+num_sets-1) = samples(app.ComParameters.gridchannels(end)+4+str2double(app.ForcechDropDown.Value),:) - app.RecordParameters.offsetforce;
                        ...
                        if k*(str2double(app.FrequencyDropDown.Value)/str2double(app.RefreshrateDropDown.Value)) <= i+num_sets-1
                            ...
                            EMGtmp{1} = filtfilt(c,d,EMG{1}(app.DecompParameters.EMGmask{idxmuscle1}==0, (k-1)*(str2double(app.FrequencyDropDown.Value)/str2double(app.RefreshrateDropDown.Value))+1:k*(str2double(app.FrequencyDropDown.Value)/str2double(app.RefreshrateDropDown.Value)))');
                            [signal.Pulsetrain{1}(:,(k-1)*(str2double(app.FrequencyDropDown.Value)/str2double(app.RefreshrateDropDown.Value))+1:k*(str2double(app.FrequencyDropDown.Value)/str2double(app.RefreshrateDropDown.Value))), Distime{1}((k-1)*(str2double(app.FrequencyDropDown.Value)/str2double(app.RefreshrateDropDown.Value))+1:k*(str2double(app.FrequencyDropDown.Value)/str2double(app.RefreshrateDropDown.Value)), :), esample2{1}] = getspikesonline(EMGtmp{1}', app.DecompParameters.extensionfactor(idxmuscle1), esample2{1}, MUfilt{1}, app.DecompParameters.norm{idxmuscle1}, noise_centroids{1}, spike_centroids{1}, (str2double(app.FrequencyDropDown.Value)/str2double(app.RefreshrateDropDown.Value)), str2double(app.FrequencyDropDown.Value));

                            EMGtmp{2} = filtfilt(c,d,EMG{2}(app.DecompParameters.EMGmask{idxmuscle2}==0, (k-1)*(str2double(app.FrequencyDropDown.Value)/str2double(app.RefreshrateDropDown.Value))+1:k*(str2double(app.FrequencyDropDown.Value)/str2double(app.RefreshrateDropDown.Value)))');
                            [signal.Pulsetrain{2}(:,(k-1)*(str2double(app.FrequencyDropDown.Value)/str2double(app.RefreshrateDropDown.Value))+1:k*(str2double(app.FrequencyDropDown.Value)/str2double(app.RefreshrateDropDown.Value))), Distime{2}((k-1)*(str2double(app.FrequencyDropDown.Value)/str2double(app.RefreshrateDropDown.Value))+1:k*(str2double(app.FrequencyDropDown.Value)/str2double(app.RefreshrateDropDown.Value)), :), esample2{2}] = getspikesonline(EMGtmp{2}', app.DecompParameters.extensionfactor(idxmuscle2), esample2{2}, MUfilt{2}, app.DecompParameters.norm{idxmuscle2}, noise_centroids{2}, spike_centroids{2}, (str2double(app.FrequencyDropDown.Value)/str2double(app.RefreshrateDropDown.Value)), str2double(app.FrequencyDropDown.Value));

                            if k < nwinbuffmax
                                DR{1}(k,:) = sum(Distime{1}(1:k*(str2double(app.FrequencyDropDown.Value)/str2double(app.RefreshrateDropDown.Value)), :) > 0, 1);
                                DR{2}(k,:) = sum(Distime{2}(1:k*(str2double(app.FrequencyDropDown.Value)/str2double(app.RefreshrateDropDown.Value)), :) > 0, 1);
                                plot(app.UIAxes4, DR{1}(1:10*str2double(app.RefreshrateDropDown.Value),idxmu1) , DR{2}(1:10*str2double(app.RefreshrateDropDown.Value),idxmu2), DR{1}(k,idxmu1) , DR{2}(k,idxmu2),'o', 'LineWidth', 4, 'MarkerSize', 20, 'Color', [0.56 0.62 0.85])
                                plot(app.UIAxes5, 1, forcefb(k)/app.RecordParameters.MVC * 100,'o', 'LineWidth', 4, 'MarkerSize', 20, 'Color', [0.9412 0.9412 0.9412])
                                line(app.UIAxes5,[0 2], [feedback(k)*0.95 feedback(k)*0.95],'LineWidth', 4, 'Color', [0.9412 0.9412 0.9412])
                                line(app.UIAxes5,[0 2], [feedback(k)*1.05 feedback(k)*1.05],'LineWidth', 4, 'Color', [0.9412 0.9412 0.9412])
                            else
                                DR{1}(k,:) = sum(Distime{1}((k-str2double(app.RefreshrateDropDown.Value)+1)*(str2double(app.FrequencyDropDown.Value)/str2double(app.RefreshrateDropDown.Value))+1:k*(str2double(app.FrequencyDropDown.Value)/str2double(app.RefreshrateDropDown.Value)), :) > 0, 1);
                                DR{2}(k,:) = sum(Distime{2}((k-str2double(app.RefreshrateDropDown.Value)+1)*(str2double(app.FrequencyDropDown.Value)/str2double(app.RefreshrateDropDown.Value))+1:k*(str2double(app.FrequencyDropDown.Value)/str2double(app.RefreshrateDropDown.Value)), :) > 0, 1);
                                DR{1}(k,:) = mean(DR{1}(k-(app.SmoothingEditField.Value-1):k,:), 1);
                                DR{2}(k,:) = mean(DR{2}(k-(app.SmoothingEditField.Value-1):k,:), 1);
                                plot(app.UIAxes4, DR{1}(k-5*str2double(app.RefreshrateDropDown.Value)+1:k+5*str2double(app.RefreshrateDropDown.Value),idxmu1) , DR{2}(k-5*str2double(app.RefreshrateDropDown.Value)+1:k+5*str2double(app.RefreshrateDropDown.Value),idxmu2), DR{1}(k,idxmu1) , DR{2}(k,idxmu2),'o', 'LineWidth', 4, 'MarkerSize', 20, 'Color', [0.56 0.62 0.85])
                                plot(app.UIAxes5, 1, forcefb(k)/app.RecordParameters.MVC * 100,'o', 'LineWidth', 4, 'MarkerSize', 20, 'Color', [0.9412 0.9412 0.9412])
                                line(app.UIAxes5,[0 2], [feedback(k)*0.95 feedback(k)*0.95],'LineWidth', 4, 'Color', [0.9412 0.9412 0.9412])
                                line(app.UIAxes5,[0 2], [feedback(k)*1.05 feedback(k)*1.05],'LineWidth', 4, 'Color', [0.9412 0.9412 0.9412])
                            end
                ...
                signal.ngrid = app.RecordParameters.ngrid;
                signal.EMGmask{1} = app.DecompParameters.EMGmask{idxmuscle1};
                signal.EMGmask{2} = app.DecompParameters.EMGmask{idxmuscle2};
                if str2double(app.ChpergridDropDown.Value) == 32
                    signal.gridname{1} = 'GR08.75MM0408';
                    signal.gridname{2} = 'GR08.75MM0408';
                else
                    signal.gridname{1} = 'GR08.75MM0808';
                    signal.gridname{2} = 'GR08.75MM0408';
                end
        ...
        function StartDRofalltheMUsButtonPushed(app, event)
            ...
            % Set the properties of the axes for data viz         
            plot(app.UIAxes4, Time(1:10*str2double(app.RefreshrateDropDown.Value)), DR(1:10*str2double(app.RefreshrateDropDown.Value),:), Time(k), DR(k,:), 'o', 'LineWidth', 4, 'MarkerSize', 20, 'Color', [0.56 0.62 0.85])
            plot(app.UIAxes5, 1, forcefb(k),'o', 'LineWidth', 4, 'MarkerSize', 20, 'Color', [0.9412 0.9412 0.9412])
            line(app.UIAxes5,[0 2], [feedback(k)*0.95 feedback(k)*0.95],'LineWidth', 4, 'Color', [0.9412 0.9412 0.9412])
            line(app.UIAxes5,[0 2], [feedback(k)*1.05 feedback(k)*1.05],'LineWidth', 4, 'Color', [0.9412 0.9412 0.9412])
            app.UIAxes4.XLim = [Time(1) Time(10*str2double(app.RefreshrateDropDown.Value))];
                ...
                while i <= nwin - str2double(app.FrequencyDropDown.Value)*5
                    ...
                    if num_sets > 0
                        ...
                        if k*(str2double(app.FrequencyDropDown.Value)/str2double(app.RefreshrateDropDown.Value)) <= i+num_sets-1
                            ...
                            if k < nwinbuffmax
                                DR(k,:) = sum(Distime{1}(1:k*(str2double(app.FrequencyDropDown.Value)/str2double(app.RefreshrateDropDown.Value)), :) > 0, 1);
                                plot(app.UIAxes4, Time(1:10*str2double(app.RefreshrateDropDown.Value)), DR(1:10*str2double(app.RefreshrateDropDown.Value),:), Time(k), DR(k,:),'o', 'LineWidth', 4, 'MarkerSize', 20, 'Color', [0.56 0.62 0.85])
                                app.UIAxes4.XLim = [Time(1) Time(10*str2double(app.RefreshrateDropDown.Value))];
                                plot(app.UIAxes5, 1, forcefb(k)/app.RecordParameters.MVC * 100,'o', 'LineWidth', 4, 'MarkerSize', 20, 'Color', [0.9412 0.9412 0.9412])
                                line(app.UIAxes5,[0 2], [feedback(k)*0.95 feedback(k)*0.95],'LineWidth', 4, 'Color', [0.9412 0.9412 0.9412])
                                ...
                                DR(k,:) = sum(Distime{1}((k-str2double(app.RefreshrateDropDown.Value)+1)*(str2double(app.FrequencyDropDown.Value)/str2double(app.RefreshrateDropDown.Value))+1:k*(str2double(app.FrequencyDropDown.Value)/str2double(app.RefreshrateDropDown.Value)), :) > 0, 1);
                                DR(k,:) = mean(DR(k-(app.SmoothingEditField.Value-1):k,:), 1);
                                plot(app.UIAxes4, Time(k-5*str2double(app.RefreshrateDropDown.Value)+1:k+5*str2double(app.RefreshrateDropDown.Value)), DR(k-5*str2double(app.RefreshrateDropDown.Value)+1:k+5*str2double(app.RefreshrateDropDown.Value),:), Time(k), DR(k,:),'o', 'LineWidth', 4, 'MarkerSize', 20, 'Color', [0.56 0.62 0.85])
                                app.UIAxes4.XLim = [Time(k-5*str2double(app.RefreshrateDropDown.Value)+1) Time(k+5*str2double(app.RefreshrateDropDown.Value))];
                                plot(app.UIAxes5, 1, forcefb(k)/app.RecordParameters.MVC * 100,'o', 'LineWidth', 4, 'MarkerSize', 20, 'Color', [0.9412 0.9412 0.9412])
                                line(app.UIAxes5,[0 2], [feedback(k)*0.95 feedback(k)*0.95],'LineWidth', 4, 'Color', [0.9412 0.9412 0.9412])
                                line(app.UIAxes5,[0 2], [feedback(k)*1.05 feedback(k)*1.05],'LineWidth', 4, 'Color', [0.9412 0.9412 0.9412])
                            end
        ...
        % Button pushed function: InitializeSAGAButton_2
        function InitializeSAGAButton_2Pushed(app, event)
            % If the script has been terminated manually, it can occur that the device
            % keeps sampling. To be sure, a lib.cleanUp() is performed prior to running
            % the example.
            if exist('lib', 'var')
        ...
        % Button pushed function: DisconnectSAGAButton_2
        function DisconnectSAGAButton_2Pushed(app, event)
            % Stop sampling on the device
            app.ComParameters.device.stop();
        ...
        function ImportdataButtonPushed(app, event)
            ...
            if contains(app.filename2,'edited')
                files = load([app.pathname2 app.filename2], 'signal', 'edition'); 
                ...
                files = load([app.pathname2 app.filename2], 'signal');   
                for i = 1:files.signal.ngrid
                    % Update the list and load the edited files
            ...
            files.parameters.nbelectrodes = str2double(app.ChpergridDropDown.Value);
        ...
        function ReevaluatewindowButtonPushed(app, event)
            ...
            [c,d] = butter(3,10/(str2double(app.FrequencyDropDown.Value)/2),'high');
            EMG = filtfilt(c,d,EMG');
            EMG = EMG(idx,app.MUedition.signal.EMGmask{str2double(C{2})}==0)';
    ...
    methods (Access = private)
        ...
        function createComponents(app)
            ...
            % Create DisconnectSAGAButton_2
            app.DisconnectSAGAButton_2 = uibutton(app.VisualizationBiofeedback, 'push');
            app.DisconnectSAGAButton_2.ButtonPushedFcn = createCallbackFcn(app, @DisconnectSAGAButton_2Pushed, true);
            app.DisconnectSAGAButton_2.BackgroundColor = [0.149 0.149 0.149];
            app.DisconnectSAGAButton_2.FontName = 'Avenir Next';
            app.DisconnectSAGAButton_2.FontSize = 14;
            app.DisconnectSAGAButton_2.FontColor = [1 0 0];
            app.DisconnectSAGAButton_2.Enable = 'off';
            app.DisconnectSAGAButton_2.Position = [733 640 179 26];
            app.DisconnectSAGAButton_2.Text = 'Disconnect SAGA';
            ...
            % Create InitializeSAGAButton_2
            app.InitializeSAGAButton_2 = uibutton(app.VisualizationBiofeedback, 'push');
            app.InitializeSAGAButton_2.ButtonPushedFcn = createCallbackFcn(app, @InitializeSAGAButton_2Pushed, true);
            app.InitializeSAGAButton_2.BackgroundColor = [0.149 0.149 0.149];
            app.InitializeSAGAButton_2.FontName = 'Avenir Next';
            app.InitializeSAGAButton_2.FontSize = 14;
            app.InitializeSAGAButton_2.FontColor = [0.3922 0.8314 0.0745];
            app.InitializeSAGAButton_2.Position = [545 640 182 26];
            app.InitializeSAGAButton_2.Text = 'Initialize SAGA';
            ...
            % Create DisconnectSAGAButton
            app.DisconnectSAGAButton = uibutton(app.VisualizationPanel, 'push');
            app.DisconnectSAGAButton.ButtonPushedFcn = createCallbackFcn(app, @DisconnectSAGAButtonPushed, true);
            app.DisconnectSAGAButton.BackgroundColor = [0.149 0.149 0.149];
            app.DisconnectSAGAButton.FontName = 'Avenir Next';
            app.DisconnectSAGAButton.FontSize = 14;
            app.DisconnectSAGAButton.FontColor = [1 0 0];
            app.DisconnectSAGAButton.Enable = 'off';
            app.DisconnectSAGAButton.Position = [730 641 176 26];
            app.DisconnectSAGAButton.Text = 'Disconnect SAGA';

            % Create InitializeSAGAButton
            app.InitializeSAGAButton = uibutton(app.VisualizationPanel, 'push');
            app.InitializeSAGAButton.ButtonPushedFcn = createCallbackFcn(app, @InitializeSAGAButtonPushed, true);
            app.InitializeSAGAButton.BackgroundColor = [0.149 0.149 0.149];
            app.InitializeSAGAButton.FontName = 'Avenir Next';
            app.InitializeSAGAButton.FontSize = 14;
            app.InitializeSAGAButton.FontColor = [0.3922 0.8314 0.0745];
            app.InitializeSAGAButton.Enable = 'off';
            app.InitializeSAGAButton.Position = [503 640 211 26];
            app.InitializeSAGAButton.Text = 'Initialize SAGA';
            ...
            app.FrequencyDropDown.Items = {'512', '1024', '2048', '4096'};
            ...
            app.FrequencyDropDown.Value = '4096';
            ...
            % Create ChpergridDropDownLabel
            app.ChpergridDropDownLabel = uilabel(app.RecordingSettingsPanel);
            app.ChpergridDropDownLabel.HorizontalAlignment = 'right';
            app.ChpergridDropDownLabel.FontName = 'Avenir Next';
            app.ChpergridDropDownLabel.FontSize = 14;
            app.ChpergridDropDownLabel.FontColor = [0.8118 0.502 1];
            app.ChpergridDropDownLabel.Position = [5 399 74 22];
            app.ChpergridDropDownLabel.Text = 'Ch per grid';

            % Create ChpergridDropDown
            app.ChpergridDropDown = uidropdown(app.RecordingSettingsPanel);
            app.ChpergridDropDown.Items = {'32', '64'};
            app.ChpergridDropDown.ValueChangedFcn = createCallbackFcn(app, @ChpergridDropDownValueChanged, true);
            app.ChpergridDropDown.FontName = 'Avenir Next';
            app.ChpergridDropDown.FontSize = 14;
            app.ChpergridDropDown.FontColor = [0.8118 0.502 1];
            app.ChpergridDropDown.BackgroundColor = [0.149 0.149 0.149];
            app.ChpergridDropDown.Position = [94 397 116 25];
            app.ChpergridDropDown.Value = '32';
            ...
            app.RefreshrateDropDownLabel.Position = [-3 430 82 22];
            ...
            app.RefreshrateDropDown.Items = {'2', '4', '8', '16', '32'};
            ...
            app.RefreshrateDropDown.Position = [94 428 116 25];
            app.RefreshrateDropDown.Value = '8';

            % Create DockcomDropDownLabel
            app.DockcomDropDownLabel = uilabel(app.RecordingSettingsPanel);
            app.DockcomDropDownLabel.HorizontalAlignment = 'right';
            app.DockcomDropDownLabel.FontName = 'Avenir Next';
            app.DockcomDropDownLabel.FontSize = 14;
            app.DockcomDropDownLabel.FontColor = [0.8118 0.502 1];
            app.DockcomDropDownLabel.Position = [12 369 67 22];
            app.DockcomDropDownLabel.Text = 'Dock com';

            % Create DockcomDropDown
            app.DockcomDropDown = uidropdown(app.RecordingSettingsPanel);
            app.DockcomDropDown.Items = {'electrical', 'optical', 'wifi'};
            app.DockcomDropDown.FontName = 'Avenir Next';
            app.DockcomDropDown.FontSize = 14;
            app.DockcomDropDown.FontColor = [0.8118 0.502 1];
            app.DockcomDropDown.BackgroundColor = [0.149 0.149 0.149];
            app.DockcomDropDown.Position = [94 367 116 25];
            app.DockcomDropDown.Value = 'optical';

            % Create ForcechDropDownLabel
            app.ForcechDropDownLabel = uilabel(app.RecordingSettingsPanel);
            app.ForcechDropDownLabel.HorizontalAlignment = 'right';
            app.ForcechDropDownLabel.FontName = 'Avenir Next';
            app.ForcechDropDownLabel.FontSize = 14;
            app.ForcechDropDownLabel.FontColor = [0.8118 0.502 1];
            app.ForcechDropDownLabel.Position = [21 340 59 22];
            app.ForcechDropDownLabel.Text = 'Force ch';

            % Create ForcechDropDown
            app.ForcechDropDown = uidropdown(app.RecordingSettingsPanel);
            app.ForcechDropDown.Items = {'1', '2', '3'};
            app.ForcechDropDown.FontName = 'Avenir Next';
            app.ForcechDropDown.FontSize = 14;
            app.ForcechDropDown.FontColor = [0.8118 0.502 1];
            app.ForcechDropDown.BackgroundColor = [0.149 0.149 0.149];
            app.ForcechDropDown.Position = [94 337 116 25];
            app.ForcechDropDown.Value = '1';
    ...
    methods (Access = public)
        ...
        % Construct app
        function app = ISpin_SAGA_exported
